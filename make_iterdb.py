...
if 1 == 0:
    ...
    comparePlot(psipni,ni,psi0,ni0,'ni','ni raw','psip','10^20 m^-3')
    comparePlot(psipne,ne,psi0,ne0,'ne','ne raw','psip','10^20 m^-3')
    comparePlot(psipte,te,psi0,te0,'te','te raw','psip','keV')
    comparePlot(psipti,ti,psi0,ti0,'ti','ti raw','psip','keV')

zeff = (ni0 + nz0 * impurityCharge**2) / ne0 
if 1 == 1:
    singlePlot(psi0, zeff, '', 'psip', 'zeff')
if 1 == 0:
    plt.plot(psi0,ne0,label='ne (10^20 m^3)')
    plt.plot(psi0,ni0,label='ni (10^20 m^3)')
    plt.plot(psi0,te0,label='te (KeV)')
    plt.plot(psi0,nz0*6.,label='nz*6')
    plt.plot(psi0,ti0,label='ti (KeV)')
